...
const getZoneAccessInfo = (zoneName: string, hour: number): string => {
  switch (zoneName) {
    case "Main Ceremonial Area":
      if (hour >= 4 && hour <= 22) {
        return "Open to all devotees. High crowd expected during ceremony times (5-8 AM, 6-8 PM).";
      }
      return "Limited night access. Security checkpoints active.";
    case "Accommodation Zone":
      if (hour >= 22 || hour <= 6) {
        return "Residents only. Enhanced security patrol active.";
      }
      return "Check-in/out at zone gates. Valid accommodation pass required.";
    case "Parking & Transport Zone":
      if ((hour >= 7 && hour <= 10) || (hour >= 16 && hour <= 19)) {
        return "Peak hours. Use designated entry/exit points. Follow traffic guides.";
      }
      return "Regular flow. Shuttle service every 15 minutes.";
    case "Restricted Area":
      return "Special permit required. Entry monitored 24/7.";
    default:
      return "Follow posted guidelines and security instructions.";
  }
};

const calculateAreaBoundary = (basePoints: [number, number][], timeOffset: number, crowdFactor: number): L.LatLngExpression[] => {
  return basePoints.map(([lat, lng], index) => {
    const angle = (index / basePoints.length) * Math.PI * 2 + timeOffset;
    const dynamicFactor = 0.0002 * crowdFactor;
    const pulseEffect = Math.sin(timeOffset * 2) * 0.0001;

    // Add dynamic movement to boundaries
    return [
      lat + (Math.sin(angle) * dynamicFactor) + pulseEffect,
      lng + (Math.cos(angle) * dynamicFactor) + pulseEffect
    ] as L.LatLngExpression;
  });
};

const defineKumbhZones = (hour: number, timeOffset: number): AreaZone[] => {
  return [
    {
      ...
      basePoints: [
        ...
        [20.0045, 73.7902],
        [20.0050, 73.7916],
        [20.0067, 73.7920], // Close the polygon
      ],
      color: "#7c3aed",
      status: "Busy",
    ...
    },
    {
      ...
      basePoints: [
        ...
        [20.0105, 73.7930],
        [20.0110, 73.7945],
        [20.0130, 73.7950], // Close the polygon
      ],
      color: "#ea580c",
    ...
    },
    {
      ...
      basePoints: [
        ...
        [20.0020, 73.7885],
        [20.0025, 73.7900],
        [20.0040, 73.7910], // Close the polygon
      ],
      color: "#0d9488",
    ...
    },
    {
      ...
      basePoints: [
        ...
        [20.0055, 73.7895],
        [20.0060, 73.7905],
        [20.0070, 73.7910], // Close the polygon
      ],
      color: "#dc2626",
