...
import { 
  ...
  ExternalLink, Phone, Timer, Info, Accessibility
} from 'lucide-react';
...
export function SmartTransportationHub() {
  ...
  const [transportGuideExpanded, setTransportGuideExpanded] = useState(false);
  const [transportGuideTab, setTransportGuideTab] = useState("bus");
            ...
            <div className="mb-4">
              <h2 className="text-xl font-semibold mb-4 text-[#FF7F00] flex items-center justify-between">
                ...
                <Button 
                  variant="outline" 
                  size="sm"
                  onClick={() => setTransportGuideExpanded(!transportGuideExpanded)}
                  className="text-xs"
                >
                  {transportGuideExpanded ? "Show Less" : "Show More"}
                </Button>
              </h2>
              
              {transportGuideExpanded ? (
                <Tabs defaultValue={transportGuideTab} onValueChange={setTransportGuideTab} className="w-full">
          ...
          <TabsContent value="emergency" className="space-y-4">
            <div className="mb-4 p-3 bg-red-50 rounded-lg text-sm">
              <div className="flex gap-2 items-center">
                ...
                <span className="font-medium">Emergency transportation and services during Kumbh Mela</span>
              </div>
            </div>
